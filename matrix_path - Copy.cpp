...
void shortestPathRecursive(int* predecessor, int destination);
   ...
   while (NUMBER_OF_ALLOWED_NODES < NUMBER_OF_NODES) // for each node
	{
      ...
      // apendix 1         
      if(allowed[destination] == 1) //                         <--- COMMENT OUT TO FIND FINAL ARRAY  
         break;                     //                               WHERE EACH ELEMENT OF ALLOWED = 1
      // exit on finding the shortest path to destination
	}
   
   cout << endl << "FINAL ARRAYS:" << endl; // show the end result
   print_arrays(distance, predecessor, allowed);

   cout << endl << "Shortest Distance: " << distance[destination] << endl;//  <--- Show the shortest path to the destination
   
   cout << "The path taken: "; // recursivly print the path taken to find the shortest distance to the destination
   shortestPathRecursive(predecessor, destination);  
        
   // apendix 2
   int num = 1;
   int dest = destination;
   
   do{// for each node we have gone to 
      dest = predecessor[dest]; // go to the previous node
      num++;                    // count the edge of those nodes
   }while(predecessor[dest] != start); 

   if(num == NUMBER_OF_NODES-1) // if the number of edges is the same as the number of nodes
      cout << endl << "This path is Hamiltonian" << endl; // the path is Hamiltonian 
...
int find_next(int* distance, int* allowed)
	{
		int min_distance, i, next;
         // to find the max distance change this to 0 and change the sign in the if statement so "distance[i] > min_distance"
		min_distance = INF;        // this does not work because the path is infinite i think
...
void  shortestPathRecursive(int* predecessor, int destination) // this function recursivly prints the order that each node
{                                                              // was visited when finding the shortest path
   
   if(predecessor[destination] != destination) // if we are not at the end of the list 
         shortestPathRecursive(predecessor, predecessor[destination]); // call the function again
         
   cout << destination << ", ";// we now print the current node 
